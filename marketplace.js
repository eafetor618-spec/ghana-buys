...
(async function initNavAccountLink() {
  ...
  async function refreshUnreadBadge(userId) {
    ...
    const [msgRes, featureRes, storeRes, idRes, postRes] = await Promise.all([
      sb.from('messages').select('id', { count: 'exact', head: true }).eq('recipient_id', userId).eq('read', false),
      sb.from('feature_requests').select('id', { count: 'exact', head: true }).eq('user_id', userId).is('seen_at', null).not('reviewed_at', 'is', null),
      sb.from('store_subscription_requests').select('id', { count: 'exact', head: true }).eq('user_id', userId).is('seen_at', null).not('reviewed_at', 'is', null),
      sb.from('id_verifications').select('user_id', { count: 'exact', head: true }).eq('user_id', userId).is('seen_at', null).not('reviewed_at', 'is', null),
      sb.from('listing_post_requests').select('id', { count: 'exact', head: true }).eq('user_id', userId).is('seen_at', null).not('reviewed_at', 'is', null),
    ]);

    const total = (msgRes.count || 0) + (featureRes.count || 0) + (storeRes.count || 0) + (idRes.count || 0) + (postRes.count || 0);
